...
class Vocabulary:
    ...
    def add(self, token):
        ...
        self.token2idx[token] = self._current_id
        self.idx2token[self._current_id] = token
        self._current_id += 1
...
def save_vocabulary(vocabulary, output_file):
    ...
    Args:
        vocabulary: Vocabulary object to save
        output_file: Path to output file
    """
    # Get all tokens in index order (0, 1, 2, ...)
    tokens = [vocabulary.idx2token[i] for i in range(len(vocabulary))]
